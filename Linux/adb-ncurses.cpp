...
void ADBstate(){
halfdelay(1);
char statusDC[20],modelDC[20],brandDC[20],cpuDC[20],displayDC[20],operatorDC[20],batteryDC[20],command[255];
fstream statusD,modelD,brandD,cpuD,operatorD,displayD,batteryD;
...
mvprintw(row/2+2,cal/2+tmp,"Update adb-ncurses");
if(y[2]==7) attron(COLOR_PAIR(1));
else attron(COLOR_PAIR(2));
mvprintw(row/2+3,cal/2+tmp,"Back");
}
