...
clear();
char one[90],two[256];
if(y==1) printw("directory recovery:");
if(y==2) printw("directory boot:");
if(y==3) printw("directory system:");
if(y==4) printw("directory userdata:");
if(y==5) printw("directory radio:");
if(y==6) printw("directory update:");
getstr(one);
if(y==1) sprintf(two,"fastboot flash recovery %s",one);
if(y==2) sprintf(two,"fastboot flash boot %s",one);
if(y==3) sprintf(two,"fastboot flash system %s",one);
if(y==4) sprintf(two,"fastboot flash userdata %s",one);
if(y==5) sprintf(two,"fastboot flash radio %s",one);
if(y==6) sprintf(two,"fastboot update %s",one);
system(two);
}
break;
}
...
if(y==2) printw("directory boot:");
getstr(one);
sprintf(two,"fastboot boot %s",one);
system(two);
}
break;
}
...
printf("ip addres: ");
cin >> one;
sprintf(two,"adb connect %s:5555",one);
...
mvprintw(row/2+6,cal/2-12,"please connect device");
getch();
}
else{
mvprintw(row/2,cal/2-1,"ADB");
mvprintw(row/2+1,cal/2-12,"please connect device");
mvprintw(row-1,0,"beta");
getch();
halfdelay(1);
for(int i=0;i<3;i++){
